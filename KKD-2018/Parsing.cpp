...
namespace P
{
	...
	bool checkExpression(FST::FSA fsa, LT::Entry &LT_struct, IT::IdTable &idtable, IT::Entry &IT_struct, In::element ml, int i, std::stack<int>&PIN, Error::ERRORS& table)
	{
		...
		if (rc)
		{
			...
			/*if (fsa.lexema == LEX_DECLARE)
			{
				int y = 0;
				for (int x = 0; x < idtable.size; x++)
				{
					if (!strcmp(ml.mtxt[i + 2], idtable.table[x].id_buf))
 						y++;
				}
				if (y>1)
					table.pushError(6, LT_struct.sn);
				y = 0;
			}*/
			if (fsa.lexema == LEX_RAVNO)
			{
			

				int x = i;
				for (int a = 0; a < idtable.size; a++)
				{
					if (!strcmp(ml.mtxt[x - 1], idtable.table[a].id_buf))
						if (idtable.table[a].iddatatype == IT::INT)
							while (strcmp(ml.mtxt[x], ";"))
							{
								for (int z = 0; z < idtable.size; z++)
								{
									if (!strncmp(ml.mtxt[x], "\"", 1))
										table.pushError(6, LT_struct.sn);
									if (!strcmp(ml.mtxt[x], idtable.table[z].id_buf))
									{
										if (idtable.table[z].iddatatype != IT::INT)
											table.pushError(6, LT_struct.sn);
									}
								}
								x++;
				...
				{
					if (!strcmp(ml.mtxt[x - 1], idtable.table[a].id_buf))
						if (idtable.table[a].iddatatype == IT::STR)
							while (strcmp(ml.mtxt[x], ";"))
							{
								for (int z = 0; z < idtable.size; z++)
								{
									...
									if (!strcmp(ml.mtxt[x], idtable.table[z].id_buf))
									{
										if (idtable.table[z].iddatatype != IT::STR)
											table.pushError(6, LT_struct.sn);
									}
									if (idtable.table[z].value.vint == atoi(ml.mtxt[x]))
										table.pushError(6, LT_struct.sn);
								}
								x++;
							}
			...
			else
			{
				if (fsa.lexema == LEX_LITERAL) // если литерал
				{
					...
					if (fsa.CID == LT::LT_INT)
					{
						...
						if (strcmp(ml.mtxt[i], "0") == 0 && strcmp(ml.mtxt[i - 1], "/") == 0)
						{
							table.pushError(6, LT_struct.sn);
						}
	...
	LEX Par(In::IN in, Error::ERRORS& table, Parm::PARM parm, bool &flib, Log::LOG log)
	{
		...
		if (flib)
		{
			...
			IT::Entry sqr;
            sqr.idxfirstLE = 0;
			strcpy(sqr.id_buf, "sqr"); strcpy(sqr.id, "sqr");
			sqr.iddatatype = IT::IDDATATYPE(1); sqr.idtype = IT::IDTYPE(3);
			sqr.is_to = true;
			IT::Add(Tables.Idtable, sqr);
			IT::Entry doub;
			doub.idxfirstLE = 0;
			strcpy(doub.id_buf, "doub");strcpy(doub.id, "doub");
			doub.iddatatype = IT::IDDATATYPE(1); doub.idtype = IT::IDTYPE(3);
			doub.is_to = true;
			IT::Add(Tables.Idtable, doub);
		}
